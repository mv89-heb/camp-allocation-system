...
DAMAGE_OUTPUT_FIELDS = (
    "id", "apartment", "category", "subcategory", "item_name", "severity", "status", "description", "estimated_cost",
...
def damage_summary(
    ...
    rows = db.scalars(query.order_by(DamageReportDB.updated_at.desc()).limit(2000)).all()
    groups: dict[tuple[str, str | None, str | None, str], int] = {}
    for row in rows:
        key = (row.category, row.subcategory, row.item_name, row.status)
        groups[key] = groups.get(key, 0) + 1
    return [
        {"category": k[0], "subcategory": k[1], "item_name": k[2], "status": k[3], "count": count}
        for k, count in sorted(groups.items(), key=lambda pair: (-pair[1], str(pair[0])))
    ]
