...
@router.get("/rooms")
def field_rooms(db: Session = Depends(get_db)):
    ...
    out=[]
    for name in names:
        req=requirements.get(name); act=actuals.get(name)
        out.append({
            "apartment": name,
...
def field_inventory(data: ActualInventoryUpdate, db: Session = Depends(get_db)):
    ...
    if record is None:
        record=ActualDB(apartment=data.apartment,**values); db.add(record)
    else:
        for f,v in values.items(): setattr(record,f,v)
    record.checked_at=utc_now(); record.checked_by="field-reporter"; db.commit()
    return {"status":"success","apartment":data.apartment,"checked_at":record.checked_at}

@router.post("/damages", status_code=201)
def field_damage(data: DamageCreateRequest, db: Session = Depends(get_db)):
    ...
    if not exists: raise HTTPException(status_code=404, detail="Room/apartment does not exist")
    now=utc_now(); row=DamageReportDB(apartment=data.apartment,category=data.category,severity=data.severity,status="OPEN",description=data.description,estimated_cost=data.estimated_cost,responsible_party=data.responsible_party,resolution_notes=data.resolution_notes,evidence_urls=data.evidence_urls,reported_by="field-reporter",reported_at=now,updated_by="field-reporter",updated_at=now)
